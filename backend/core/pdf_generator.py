...
def _icon_chart(size=48):
    """Bar chart icon for the visual analysis section."""
    d = Drawing(size, size)
    d.add(Rect(2, 2, size-4, size-4, rx=6, ry=6, fillColor=SOFT_TEAL, strokeColor=TEAL, strokeWidth=1.2))
    # Three rising bars
    bar_data = [(8, 10, 14), (20, 20, 14), (32, 30, 14)]
    bar_colors = [TEAL, BRAND, ACCENT]
    for (x, h, w_), bc in zip(bar_data, bar_colors):
        d.add(Rect(x, 6, w_ - 2, h, fillColor=bc, strokeColor=None))
    # X axis line
    d.add(Line(6, 6, size-6, 6, strokeColor=MUTED, strokeWidth=1.2))
    return _IconFlowable(d, size, size)
...
class PDFReportGenerator:
    ...
    def _parse(self, results):
        ...
        return {
            ...
            'at':    _now_colombo(),
            '_raw_results': results,  # kept for chart generation
        }
    ...
    def _visual_analysis(self, s, raw_results: dict) -> list:
        """Embed charts generated by ChartEngine as a dedicated report section."""
        if not raw_results:
            return []
        ...
        try:
            ...
            try:
                # Fallback: try importing directly (when running outside the package)
                import sys, os
        ...
        # Only include charts that actually generated content
        available = {k: v for k, v in charts.items() if v is not None}
        if not available:
            return []

        def _chart_title(key: str) -> str:
            return {
        ...
        def _png_to_image(png_bytes: bytes, max_width: float) -> Image:
            ...
            # Use a separate buffer for PIL so its lazy-loader doesn't
            # corrupt the stream that ReportLab needs to render later.
        ...
        it = [
            ...
            Paragraph(
                "Charts generated from the pre-computed analysis. Each chart adapts to your "
                "dataset size and is skipped automatically when the underlying data is absent "
                "or trivial.",
                s['b']
            ),
            Spacer(1, 10),
        ...
        # ── Layout: radar always right-floated next to missing/target ─────────
        # Preferred order for maximum narrative flow
        order = [
            'data_health_radar',
            'missing_values',
            'target_distribution',
        ...
        if radar_bytes and companion_keys:
            ...
            try:
                ...
                companion_img = _png_to_image(companion_bytes, CW * 0.58)
                side_table = Table(
                ...
                it.append(side_table)

                # Captions under each
                cap_table = Table(
                    [[Paragraph(_chart_caption(companion_key), s['cap']),
                      Paragraph(_chart_caption('data_health_radar'), s['cap'])]],
                    colWidths=[CW * 0.60, CW * 0.40],
                )
        ...
        # ── Remaining charts — full-width, one per block ───────────────────
        for key in order:
            if key in rendered or key not in available:
                continue
            try:
                img = _png_to_image(available[key], CW)
                title = _chart_title(key)
                caption = _chart_caption(key)

                header = Table(
                    [[Paragraph(f"<b>{title}</b>", s['sub'])]],
                    colWidths=[CW],
                )
